...
MODULE_DESCRIPTION("Lab target module");

// The secrets you're trying to leak!
static volatile char __attribute__((aligned(32768))) kernel_secret3[LAB_SECRET_MAX_LEN] = "MIT{h4rd3st}";
static volatile char __attribute__((aligned(32768))) kernel_secret2[LAB_SECRET_MAX_LEN] = "MIT{scary_sp3ctr3!}";
static volatile char __attribute__((aligned(32768))) kernel_secret1[LAB_SECRET_MAX_LEN] = "MIT{k3rn3l_m3m0r135}";
...
static struct proc_dir_entry *lab_procfs_victim = NULL;
static const struct file_operations  lab_victim_ops = {
    .write = lab_victim_write,
    .read = lab_victim_read,
};
 ...
 *
 * Arguments:
 *  - cmd: A pointer to a lab_command object to the kernel log
 *
 * Returns: None
 * Side Effects: Prints to kernel log
 */
void print_cmd(lab_command *cmd) {
    if (NULL == cmd) return;
 ...
 */
int lab_init(void) {
    printk(KERN_INFO "[lab] Hello, World\n");
    lab_procfs_victim = proc_create("lab-victim", 0, NULL, &lab_victim_ops);
    return 0;
}
 ...
 */
void lab_fini(void) {
    printk(KERN_INFO "[lab] Goodbye!\n");
    proc_remove(lab_procfs_victim);
}

/*
 * lab_victim_read
 * procfs read handler that does nothing so that reading from /proc/lab-victim
 * can actually return.
 */
 ...
 * Writes expect the user to write a lab_command struct to the module.
 *
 * Input: A lab_command struct for us to parse.
...
ssize_t lab_victim_write(struct file *file_in, const char __user *userbuf, size_t num_bytes, loff_t *offset) {
    lab_command user_cmd;
    struct page *pages[LAB_SHARED_MEMORY_NUM_PAGES];
    char *kernel_mapped_region[LAB_SHARED_MEMORY_NUM_PAGES];
    int retval;
    int i, j, z;
    char secret_data;
    volatile char tmp;
    ...
    for (i = 0; i < LAB_SHARED_MEMORY_NUM_PAGES; i++) {
        pages[i] = NULL;
    }

    if (copy_from_user(&user_cmd, userbuf, sizeof(user_cmd)) == 0) {
        // arg1 is always a pointer to the shared memory region
        if (!access_ok(user_cmd.arg1, LAB_SHARED_MEMORY_SIZE)) {
            printk(KERN_INFO "[lab] Invalid user request- shared memory is 0x%llX\n", user_cmd.arg1);
            return num_bytes;
        }

        // arg2 is always the secret index to use
        if (!(user_cmd.arg2 < LAB_SECRET_MAX_LEN)) {
            printk(KERN_INFO "[lab] Tried to access a secret that is too large! Requested offset %llu\n", user_cmd.arg2);
            return num_bytes;
        }

        // Pin the pages to RAM so they aren't swapped to disk
        retval = get_user_pages_fast(user_cmd.arg1, LAB_SHARED_MEMORY_NUM_PAGES, FOLL_WRITE, pages);
        if (LAB_SHARED_MEMORY_NUM_PAGES != retval) {
            printk(KERN_INFO "[lab] Unable to pin the user pages! Requested %d pages, got %d\n", LAB_SHARED_MEMORY_NUM_PAGES, retval);
        ...
        for (i = 0; i < LAB_SHARED_MEMORY_NUM_PAGES; i++) {
            ...
            if (NULL == kernel_mapped_region[i]) {
                printk(KERN_INFO "[lab] Unable to map page %d\n", i);
        ...
        switch (user_cmd.kind) {
            ...
            case COMMAND_PART1:
                ...
                if (secret_data < LAB_SHARED_MEMORY_NUM_PAGES) {
        ...
        for (i = LAB_SHARED_MEMORY_NUM_PAGES - 1; i >= 0; i--) {
            kunmap(pages[i]);
        }

        // Unpin to ensure refcounts are valid
        put_user_pages(pages, LAB_SHARED_MEMORY_NUM_PAGES);
